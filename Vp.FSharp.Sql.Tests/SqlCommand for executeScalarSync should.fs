module Vp.FSharp.Sql.Tests.``SqlCommand for executeScalarSync should``
...
let ``open and then close the connection if initially closed`` () =
    ...
    let outcome =
        ...
        |> SqlCommand.noLogger
        |> SqlCommand.executeScalarSync connection deps conf
...
let ``log for all events on globalLogger if connection initially closed`` () =
    ...
    let outcome =
        SqlCommand.text "select 1" |> SqlCommand.executeScalarSync connection deps conf
