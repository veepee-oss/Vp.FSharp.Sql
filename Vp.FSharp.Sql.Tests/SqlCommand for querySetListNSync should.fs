...
let private toFieldName = sprintf "id%i"

let private boxes values = List.map box values
...
let private readValueByIndex (columns: int32 list list) indexColumn _ (reader: SqlRecordReader<DbDataReader>) =
    columns.[indexColumn] |> List.map (reader.Value >> int32)
...
let ``have querySetListSync should open and then close the connection if initially closed and access value by columnName``
    ...
    let data =
        Mocks.fakeData
            [ [ 1..6 ] |> boxes |> List.splitInto 2; [ 1..8 ] |> boxes |> List.splitInto 2 ]
    ...
    let r0 =
        ...
        |> SqlCommand.noLogger
        |> SqlCommand.querySetListSync connection deps conf (readValueByIndex columnsIndex 0)
