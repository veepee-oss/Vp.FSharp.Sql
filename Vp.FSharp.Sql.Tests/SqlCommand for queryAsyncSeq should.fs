...
let ``open and then close the connection if initially closed and access value by columnName`` () =
    ...
    async {
        ...
        let outcome =
            ...
            |> SqlCommand.noLogger
            |> SqlCommand.queryAsyncSeq connection deps conf (readValueByFieldName ([ 2; 1; 0 ] |> toFieldNames))
...
let ``open and then close the connection if initially closed and access value by ordinal`` () =
    ...
    async {
        ...
        let outcome =
            SqlCommand.text "select 1"
            |> SqlCommand.noLogger
            |> SqlCommand.queryAsyncSeq connection deps conf (readValueByIndex [ 2; 1; 0 ])
            |> AsyncSeq.toListSynchronously
...
let ``open and then close the connection if initially closed and access valueOrNone by columnName`` () =
    ...
    async {
        ...
        let outcome =
            SqlCommand.text "select 1"
            |> SqlCommand.noLogger
            |> SqlCommand.queryAsyncSeq connection deps conf (readValueOrNoneByFieldName (toFieldNames [ 2; 1; 0 ]))
            |> AsyncSeq.toListSynchronously
            |> List.sortBy (fun values -> snd values.[0])
...
let ``open and then close connection if initially closed and access valueOrNone by ordinal`` () =
    ...
    async {
        ...
        let outcome =
            SqlCommand.text "select 1"
            |> SqlCommand.noLogger
            |> SqlCommand.queryAsyncSeq connection deps conf (readValueOrNoneByIndex [ 2; 1; 0 ])
            |> AsyncSeq.toListSynchronously
            |> List.sortBy (fun values -> values.[0])
...
let ``when using a multiple result set and one or more is empty, get all values available`` () =
    ...
    async {
        ...
        let! outcome =
            SqlCommand.text "select 1"
            |> SqlCommand.noLogger
            |> SqlCommand.queryAsyncSeq connection deps conf (fun resultSetIndex rowIndex ->
