...
type IsDisposedDbConnection() =
    ...
    interface IIsDisposed with
        member this.IsDisposed = isDisposed

    member this.CheckDisposed(value) =
        if isDisposed then
            raise (ObjectDisposedException("Connection already disposed"))
        else
            value
...
type IsDisposedDbCommand() =
    ...
    member this.CheckDisposed(value) =
        if isDisposed then
            raise (ObjectDisposedException("Command already disposed"))
        else
            value
...
type IsDisposedDbDataReader() =
    ...
    member this.CheckDisposed(value) =
        if isDisposed then
            raise (ObjectDisposedException("Data Reader already disposed"))
        else
            value
    ...
    { Columns = columns
      GetValues =
        fun resultSetIndex rowIndex ->
            ...
            | _ ->
                $"get values: out of resultSetIndex %i{resultSetIndex} or rowIndex %i{rowIndex}"
                |> failwith
      CountRows =
        fun resultSetIndex ->
...
let makeConf logger = { DefaultLogger = logger }

let makeReader data _ =
    let mutable currentRowIndex = -1
    let mutable currentResultSetIndex = 0

    { new IsDisposedDbDataReader() with
        member this.Depth = this.CheckDisposed(0)

        member this.FieldCount =
            List.length data.Columns.[currentResultSetIndex] |> this.CheckDisposed

        member this.HasRows = data.CountRows currentResultSetIndex > 0 |> this.CheckDisposed
        member this.IsClosed = this.CheckDisposed(true)
        member this.RecordsAffected = this.CheckDisposed(0)

        member this.Item
            with get (ordinal: int32): obj = this.CheckDisposed(null)

        member this.Item
            with get (name: string): obj = this.CheckDisposed(null)
        ...
        member this.GetFieldType(ordinal: int32) =
            data.Columns.[currentResultSetIndex].[ordinal].FieldType |> this.CheckDisposed

        member this.GetName(ordinal: int32) =
            data.Columns.[currentResultSetIndex].[ordinal].Name |> this.CheckDisposed
        ...
        member this.NextResult() =
            ...
            currentRowIndex <- -1
            currentResultSetIndex < data.CountResultSets |> this.CheckDisposed

        member this.Read() =
            currentRowIndex <- currentRowIndex + 1
            currentRowIndex < data.CountRows currentResultSetIndex |> this.CheckDisposed }
    :> DbDataReader
...
let makeCommand connection response =
    ...
    { new IsDisposedDbCommand() with
        ...
        member this.DbParameterCollection = this.CheckDisposed(null)
...
let makeConnectionReader cs state openCallback closeCallback response =
    ...
    { new IsDisposedDbConnection() with
        member this.ConnectionString
            with get () = this.CheckDisposed(connectionString)
            and set v = this.CheckDisposed(connectionString <- v)

        member this.Database = this.CheckDisposed("")
        member this.DataSource = this.CheckDisposed("")
        member this.ServerVersion = this.CheckDisposed("")
        member this.State = this.CheckDisposed(state)
        member this.Close() = closeCallback () |> this.CheckDisposed
        ...
        member this.BeginDbTransaction isolationLevel =
            failwith "BeginDbTransaction not called" |> this.CheckDisposed

        member this.CreateDbCommand() =
            makeCommand this response |> this.CheckDisposed }
    :> DbConnection
