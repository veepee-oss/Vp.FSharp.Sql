module Vp.FSharp.Sql.Tests.``SqlCommand for executeScalarOrNoneSync should``
...
let ``open and then close the connection if initially closed`` () =
    ...
    let outcome =
        ...
        |> SqlCommand.noLogger
        |> SqlCommand.executeScalarOrNoneSync connection deps conf

    outcome.IsSome =! true
    outcome |> Option.defaultValue 42 |> (=!) 14
...
let ``leave the connection open if initially not closed`` () =
    ...
    outcome.IsSome =! true
    outcome |> Option.defaultValue 42 |> (=!) 15
...
let ``log for all events on globalLogger if connection initially closed`` () =
    ...
    outcome.IsSome =! true
    outcome |> Option.defaultValue 42 |> (=!) 16
...
let ``log for just command events on globalLogger if connection initially not closed`` () =
    ...
    outcome.IsSome =! true
    outcome |> Option.defaultValue 42 |> (=!) 17
