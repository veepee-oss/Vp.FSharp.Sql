...
type SqlLog<'DbConnection, 'DbCommand when 'DbConnection :> DbConnection and 'DbCommand :> DbCommand> =
...
type LoggerKind<'DbConnection, 'DbCommand when 'DbConnection :> DbConnection and 'DbCommand :> DbCommand> =
...
type SqlConfiguration<'DbConnection, 'DbCommand when 'DbConnection :> DbConnection and 'DbCommand :> DbCommand> =
...
type SqlConfigurationCache<'DbConnection, 'DbCommand when 'DbConnection :> DbConnection and 'DbCommand :> DbCommand>
    private () =

    static let mutable instance: SqlConfiguration<'DbConnection, 'DbCommand> =
        SqlConfiguration.defaultValue ()

    /// Get the current state of the configuration cache (i.e. SqlConfiguration is a record, and hence immutable)
    static member Snapshot = instance

    /// Set up the logger callback
    static member Logger(value) =
        instance <- SqlConfiguration.logger value instance

    /// Set up no logger callback
    static member NoLogger() =
        instance <- SqlConfiguration.noLogger instance
...
type SqlRecordReader<'DbDataReader when 'DbDataReader :> DbDataReader>(dataReader: 'DbDataReader) =
    ...
    let cachedFieldsByName =
        cachedFields |> List.map (fun field -> (field.Name, field)) |> readOnlyDict

    let cachedFieldsByIndex =
        cachedFields |> List.map (fun field -> (field.Index, field)) |> readOnlyDict
    ...
    let failToReadFieldByName fieldName fieldTypeName =
        failwithf
            $"""Could not read field '%s{fieldName}' as %s{fieldTypeName}.
                      Available fields are %s{availableFields}"""

    let failToReadFieldByIndex fieldIndex fieldTypeName =
        failwithf
            $"Could not read field at index %d{fieldIndex} as %s{fieldTypeName}. Available fields are %s{availableFields}"
    ...
    member this.Value<'T>(fieldName: string) =
        ...
        | true, column ->
            ...
            if dataReader.IsDBNull(column.Index) && DbNull.is<'T> () then
                DbNull.retypedAs<'T> ()
            else
                dataReader.GetFieldValue<'T>(column.Index)
        | false, _ -> failToReadFieldByName fieldName typeof<'T>.Name
    ...
    member this.ValueOrNone<'T>(fieldName: string) =
        match cachedFieldsByName.TryGetValue(fieldName) with
        | true, column ->
            if dataReader.IsDBNull(column.Index) then
                None
            else
                Some(dataReader.GetFieldValue<'T>(column.Index))
        | false, _ -> failToReadFieldByName fieldName typeof<'T>.Name
    ...
    member this.Value<'T>(fieldIndex: int32) =
        ...
        | true, column ->
            // https://github.com/npgsql/npgsql/issues/2087
            if dataReader.IsDBNull(fieldIndex) && DbNull.is<'T> () then
                DbNull.retypedAs<'T> ()
            else
                dataReader.GetFieldValue<'T>(column.Index)
        | false, _ -> failToReadFieldByIndex fieldIndex typeof<'T>.Name
    ...
    member this.ValueOrNone<'T>(columnIndex: int32) =
        ...
        | true, column ->
            if dataReader.IsDBNull(column.Index) then
                ...
                Some(dataReader.GetFieldValue<'T>(column.Index))
        | false, _ -> failToReadFieldByIndex columnIndex typeof<'T>.Name
