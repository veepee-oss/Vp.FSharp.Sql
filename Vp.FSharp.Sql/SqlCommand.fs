...
let private setupCommand deps commandDefinition cancellationToken connection =
    async {
        ...
        Option.iter (deps.SetCommandTransaction command) commandDefinition.Transaction
...
let private setupConnection (connection: #DbConnection) cancellationToken =
    connection.OpenAsync(cancellationToken) |> Async.AwaitTask

let private setupConnectionSync (connection: #DbConnection) = connection.Open()
...
let rec private tryReadNextResultRecord cancellationToken (dataReader: #DbDataReader) state =
    async {
        ...
        | None ->
            let state = ReadState.nextSet state
...
let private readNextRecord cancellationToken (dataReader: #DbDataReader) state =
    async {
        ...
        if readOk then
            return ReadState.nextRecord state
        else
            return! tryReadNextResultRecord cancellationToken dataReader state
    }
...
let private readNextRecordSync (dataReader: #DbDataReader) =
    ...
    if readOk then
        ReadState.nextRecord
    else
        tryReadNextResultRecordSync dataReader

/// Execute the command and return the sets of rows as an AsyncSeq accordingly to the command definition.
/// This function runs asynchronously.
let queryAsyncSeq (connection: #DbConnection) deps conf (read: Read<_, _>) commandDefinition =
    asyncSeq {
        ...
        try
            ...
            let items =
                ...
                |> SkipFirstAsyncSeq.scanAsync
                    (fun state (dataReader, cancellationToken) -> readNextRecord cancellationToken dataReader state)
                    { Continue = true
                      SetIndex = 0
                      RecordIndex = -1 }
                ...
                |> AsyncSeq.mapAsync (fun (state, rowReader) ->
                    read state.SetIndex state.RecordIndex rowReader |> async.Return)
...
let querySeqSync (connection: #DbConnection) deps conf (read: Read<_, _>) commandDefinition =
    seq {
        let wasClosed = connection.State = ConnectionState.Closed
        let log sqlLog = log4 conf commandDefinition sqlLog
        let connectionStopwatch = Stopwatch()
        ...
        try
            ...
            let items =
                ...
                |> SkipFirstSeq.scan
                    (fun a b -> readNextRecordSync b a)
                    { Continue = true
                      SetIndex = 0
                      RecordIndex = -1 }
...
/// This function runs asynchronously.
let queryList connection deps conf (read: Read<_, _>) commandDefinition =
    queryAsyncSeq connection deps conf read commandDefinition
    |> AsyncSeq.toListAsync

/// Execute the command and return the sets of rows as a list accordingly to the command definition.
/// This function runs synchronously.
let queryListSync connection deps conf (read: Read<_, _>) commandDefinition =
    querySeqSync connection deps conf read commandDefinition |> Seq.toList

/// Execute the command and return the first set of rows as a list accordingly to the command definition.
/// This function runs asynchronously.
let querySetList (connection: #DbConnection) deps conf (read: ReadSet<_, _>) commandDefinition =
    async {
        let setList = ResizeArray()

        let readRecord setIndex recordIndex recordReader =
            if setIndex = 0 then
                setList.Add(read recordIndex recordReader)
            else
                ()

        do!
            queryAsyncSeq connection deps conf readRecord commandDefinition
            |> AsyncSeq.consume
...
/// Execute the command and return the first set of rows as a list accordingly to the command definition.
/// This function runs synchronously.
let querySetListSync (connection: #DbConnection) deps conf (read: ReadSet<_, _>) commandDefinition =
    let setList = ResizeArray()
...
/// This function runs asynchronously.
let querySetList2 connection deps conf (read1: ReadSet<_, _>) (read2: ReadSet<_, _>) commandDefinition =
    async {
        ...
        let readRecord setIndex recordIndex recordReader =
            if setIndex = 0 then
                set1List.Add(read1 recordIndex recordReader)
            elif setIndex = 1 then
                set2List.Add(read2 recordIndex recordReader)
            else
                ()
...
/// Return the 2 first sets of rows as a tuple of 2 lists accordingly to the command definition.
/// This function runs synchronously.
let querySetList2Sync connection deps conf (read1: ReadSet<_, _>) (read2: ReadSet<_, _>) commandDefinition =
    let set1List = ResizeArray()
    let set2List = ResizeArray()
    ...
    async {
        ...
        let readRecord setIndex recordIndex recordReader =
            if setIndex = 0 then
                ...
                set2List.Add(read2 recordIndex recordReader)
            elif setIndex = 2 then
                set3List.Add(read3 recordIndex recordReader)
            else
                ()
    ...
    async {
        ...
        try
            ...
            if not anyData then
                ...
                dataReader.IsDBNull(0) && DbNull.is<'Scalar> ()
            then
...
let executeScalarOrNone<'Scalar> (connection: #DbConnection) deps conf commandDefinition =
    async {
        ...
        try
            ...
            if not anyData then return raise SqlNoDataAvailableException
            else if dataReader.IsDBNull(0) then return None
...
let executeScalarOrNoneSync<'Scalar> (connection: #DbConnection) deps conf commandDefinition =
    ...
    try
        ...
        if not anyData then raise SqlNoDataAvailableException
        else if dataReader.IsDBNull(0) then None
